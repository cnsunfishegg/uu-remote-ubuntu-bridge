...
class RuntimeScriptTests(unittest.TestCase):
    ...
    def test_controller_app_is_default_and_console_remains_loopback_only(self):
        ...
        digest = (REPOSITORY / "scripts" / "runtime-source-digest").read_text()
        trace = (REPOSITORY / "scripts" / "uu_controller_trace.py").read_text()
        ...
        self.assertIn('scripts/uu_controller_trace.py"', installer)
        self.assertIn('"$HOME/.local/libexec/uu-controller-trace"', installer)
        self.assertIn('"$HOME/.local/libexec/uu-controller-trace"', uninstaller)
        ...
        self.assertIn('    trace-controller)', command)
        self.assertIn('exec /usr/bin/python3 "$controller_trace_bin" "$@"', command)
        ...
        self.assertIn('windowmap "$client_window"', console)
        self.assertIn('windowactivate --sync "$client_window"', console)
        self.assertIn("cleanup_window", console)
        self.assertIn("enable_local_viewer_drag_when_mapped", console)
        self.assertIn('stop_window_child "$window_drag_pid"', console)
        self.assertIn("InputOnly = 2", console)
        self.assertIn("x11.XMoveWindow(", console)
        ...
        self.assertIn("scripts/uu_controller_trace.py", digest)
        self.assertIn("records_key_identity", trace)
        self.assertIn('"records_text": False', trace)
...
'''
    ...
    def test_phone_ime_unicode_input_is_normalized(self):
        ...
        self.assertIn('"-loops", "0", "-verbose"', helper)
        self.assertIn("wait_for_selection_request", helper)
    ...
    def test_direct_x11_input_route_is_opt_in_and_fail_safe(self):
        ...
        private_ready = launcher.index('mv "$private_display_tmp"')
        helper_start = launcher.index("\nstart_x11_input_helper\n")
        self.assertLess(private_ready, helper_start)
