...
安装后，在 Ubuntu 上查看本机 UU 账号/设备状态，请打开桥接器生成的英文
**UU Remote** 入口；要从本机控制其他电脑，请使用单独的 **UU Remote Controller**。
控制端会先明确询问：它需要暂时停止这台 Ubuntu 的被控服务，在可见桌面原生
打开 UU；关闭控制端后自动恢复服务。如果你是通过 UU 连进这台 Ubuntu 的，
不要继续这个切换，否则自己的连接也会断开。Wine 自行生成的中文 **UU远程**
